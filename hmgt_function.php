...
function MJ_hmgt_login_link()
{
    ...
    if ( is_user_logged_in() )
    {
        ...
        <br />
        <a style="margin-left: 7%;" href="<?php echo wp_logout_url(); ?>">
            <i class="fa fa-sign-out m-r-xs"><?php esc_html_e('Logout','hospital_mgt');?>
        </a> 
        <?php 
    }
    else 
...
{
	?>
	<div class="modal-header"><?php esc_html_e('Import Data','hospital_mgt');?><a href="#" class="close-btn-cat badge badge-danger pull-right">X</a>
	</div>
	<form class="form-horizontal" action="#" method="post" enctype="multipart/form-data">
		<div class="form-group">
			<div class="mb-3 row">
...
function MJ_hmgt_add_category_type($model) 
{
	...
	?>
	<div class="modal-header"> <a href="#" model="<?php echo esc_attr($model);?>" class="<?php echo esc_attr($close_btn_name); ?> badge badge-danger pull-right">X</a>
											...
											<div class="col-md-10">
												<blockquote class="diagnosis-report">
												 ...
												 <?php if($diagnosis->attach_report!=""){?>
													<a href="<?php echo content_url().'/uploads/hospital_assets/'.$value->attach_report;?>" target="_blank" class="btn btn-default"><i class="fa fa-download"></i> <?php echo $report_type_array->category_name.' '.esc_html__('Report','hospital_mgt'); ?></a>
													<?php }
														else{?>
...
function MJ_hmgt_add_nurse_notes() 
{
	...
	$nurse=get_userdata(get_current_user_id());
	 $row='<div class="mb-3 row"><div class="col-md-10 "><blockquote id="note-'.$id.'"><b><h4>'.esc_html__('Notes by','hospital_mgt').' '.$nurse->display_name.' on '.MJ_hmgt_hmgtConvertTime(date("Y-m-d H:i:s")).'</h4></b><p>'.$data['note'].'</p></blockquote></div><div class="col-md-1 "><a class="btn-delete-note badge badge-delete" href="#" id="notex-'.$id.'" noteid='.$id.'>X</a></div></div>';
...
function MJ_hmgt_add_doctor_notes()
{
	...
	<div class="col-md-1 "><a class="btn-delete-note badge badge-delete" href="#" id="notex-'.$id.'" noteid='.$id.'>X</a></div></div>'; 
...
function MJ_hmgt_edit_diagnosisreport_name()
{
	...
	if($model == 'report_type')
	{
		...
		$data.='<td id='.$cat_id.'>
			<a class="btn-cat-update-cancel btn btn-danger" model='.$model.' href="#" id='.$cat_id.'>'.esc_html__('Cancel','hospital_mgt').'</a>
		 </td>';
		$data.='<td id='.$cat_id.'> 
		<a class="btn-cat-update btn btn-primary" model='.$model.' href="#" id='.$cat_id.'>'.esc_html__('Save','hospital_mgt').'</a>
		</td>';
	}
	elseif($model == 'operation')
	{
						...
						if(!empty($dignosis_report_data->attach_report))
						{
							...
							{	
								if($value->report_id == $report_id)
								{
									...
									$array_var .='<a href="'.content_url().'/uploads/hospital_assets/'.$attch_report_url.'" class="btn btn-default" target="_blank"><i class="fa fa-download"></i> '.$report_type_array->category_name.' '.esc_html__('Report','hospital_mgt').'</a>
