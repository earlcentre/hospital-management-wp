...
define('HMS_PAYPAL_LOG_FILE', '.ipn_results.log');

define('HMS_PAYPAL_SSL_P_URL', 'https://www.paypal.com/cgi-bin/webscr');
//define('HMS_PAYPAL_SSL_P_URL','https://www.sandbox.paypal.com/cgi-bin/webscr');
define('HMS_PAYPAL_SSL_SAND_URL','https://www.sandbox.paypal.com/cgi-bin/webscr');

class Hmgt_paypal_class {
	...
	public function submit_paypal_post() {
		...
		if(get_option('hospital_enable_sandbox') == 'yes')
		{
			$paypal_account_type = 1;//1 sendbox, 0 live;
		}
		//$paypal_account_type = test_paypal; //1 sendbox, 0 live;
		$paypal_url = $paypal_account_type ? HMS_PAYPAL_SSL_SAND_URL : HMS_PAYPAL_SSL_P_URL;
		echo "<html>\n";
 ...
 */
	public function validate_ipn() {
		...
		$paypal_url = ($_POST['test_ipn'] == 1) ? HMS_PAYPAL_SSL_SAND_URL : HMS_PAYPAL_SSL_P_URL;
	...
	private function write_to_log() {
		...
		// Write to log
		$fp = fopen ( HMS_PAYPAL_LOG_FILE , 'a' );
		fwrite ( $fp, $this->ipn_status . "\n\n" );
		fclose ( $fp ); // close file
		chmod ( HMS_PAYPAL_LOG_FILE , 0600 );
	}
