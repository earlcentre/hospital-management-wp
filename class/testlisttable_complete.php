...
function get_columns(){
  ...
  $columns = array(
	...
	'title' => esc_html__('Title','spicy-form'),
	
	'price' => esc_html__('Type','spicy-form'),	 
	 
	  'Action'    => esc_html__('Response List','spicy-form')
...
function get_bulk_actions() {
  ...
  $actions = array(
    ...
    'delete'    => esc_html__('Delete','spicy-form')
...
function column_default( $item, $column_name ) {
  ...
  switch( $column_name ) { 
	...
	case 'link':
		return '<a href="' . esc_url( menu_page_url( 'spcf-response-list', false ) ) . '&form_id='.$item->ID.'" class="add-new-h2">' . esc_html( esc_html__( 'View Response', 'spicy-form' ) ) . '</a>';
...
function column_title($item) {
  ...
  $actions = array(
            ...
            'edit'      => sprintf('<a href="?page=%s&action=%s&post=%s">'.esc_html__('Edit','spicy-form').'</a>',$_REQUEST['page'],'edit',$item->ID),

            'delete'    => sprintf('<a href="?page=%s&action=%s&post=%s">'.esc_html__('Delete','spicy-form').'</a>',$_REQUEST['page'],'delete',$item->ID),
