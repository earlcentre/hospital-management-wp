...
if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete-reply')//DELETE REPLAY
{
	...
	if($result)
	{
		wp_redirect ( admin_url().'admin.php?page=hmgt_message&tab=view_message&from='.$message_from.'&id='.$message_id.'&message=2');
	}
}
?>
<div class="mailbox-content"><!-- MAIL BOIX CONTENT DIV -->
    ...
    <div class="message-content"><!--MESSAGE CONTENT---->
		...
		<div class="message-options pull-right">
			<a class="btn btn-default" href="?page=hmgt_message&tab=view_message&id=<?php echo $_REQUEST['id'];?>&from=<?php echo $box;?>&delete=1" onclick="return confirm('<?php esc_html_e('Are you sure you want to delete this record?','hospital_mgt');?>');"><i class="fa fa-trash m-r-xs"></i><?php esc_html_e('Delete','hospital_mgt')?></a> 
	   </div>
   </div>
   <?php 
		...
		if(!empty($allreply_data))
		{
		    ...
		    ?>
				<div class="message-content"><!--MESSAGE CONTENT---->
					<p><?php 
							echo esc_html($reply->message_comment); ?><br>
						<h5>
						<?php
						...
						esc_html_e(' || ','hospital_mgt'); 	
						?>
						<span class="timeago" title="<?php echo MJ_hmgt_hmgtConvertTime($reply->created_date); 
						?>"></span>
						<span class="comment-delete">
						<a href="admin.php?page=hmgt_message&tab=view_message&action=delete-reply&from=<?php echo $_REQUEST['from'];?>&id=<?php echo esc_attr($_REQUEST['id']);?>&reply_id=<?php echo esc_attr($reply->id);?>"><?php esc_html_e('Delete','hospital_mgt');?></a></span>
						</h5> 
					</p>
				</div>
			<?php
